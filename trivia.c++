...
int ran(int i);
void show(const vector<string> &questions,const vector<vector<string>> &mcqs, int random);
char my_answer();
void check(const vector<char> &answers,int random,char ans);
...
int main(){
	...
	vector<string> questions=question_vec(); //vector of questions
	vector<vector<string>> mcqs=choices_vec(); //vector of options
	vector<char> options=answers_vec(); //vector of answers
	...
	for(int i=10;i>=1;i--){
		...
		show(questions,mcqs,random_num);//shows the question and its options
		check(options,random_num,my_answer()); //asks the user for answer and check if its right or wrong
		...
		mcqs.erase(mcqs.begin()+random_num); //removes its options
		options.erase(options.begin()+random_num); //removes the questions answer so it wont repeat
...
void show(const vector<string> &questions,const vector<vector<string>> &mcqs,int random){
	...
	cout<<"Q No: "<<q_no<<": "<<questions.at(random)<<endl;
	cout<<"Options are:"<<endl;
	cout<<"     "<<endl;
	q_no++;

	//this here display all avaliable options for the question
	for(auto x:mcqs.at(random)){
		cout<<x;
		cout<<endl;
	}
	cout<<"    "<<endl;
...
//	while(ans.length()!=1){
//		cout<<"You incompetent dumb piece of shit only enter 1 character you fool!! "<<endl;
//		cout<<"Enter your answer ";
//		cin>>ans;
//	}
...
void check(const vector<char> &answers,int random,char ans){
	...
	if(answers.at(random)==ans){
		...
		cout<<"Correct answer is "<<answers.at(random)<<endl;
		cout<<"    "<<endl;
		lives--;
